...
@tests = (
    ...
    # make a larger file for indirect2 blocks




    [ 'yes | head -n 272385 > test/yes.txt && ls -l test/yes.txt | awk \'{ print $5 }\'',
      '544770'
    ...
    [ 'ln test/hello.txt test/hard; diff test/hello.txt test/hard ; rm test/hard',
    ...
    [ 'ln -s hello.txt test/symlink && diff test/hello.txt test/symlink ; rm test/symlink',


      ''
    ],

    #22 test remove symlink


    [ 'echo symlink > test/orig.txt ; ln -s orig.txt test/symlink.txt ; rm test/orig.txt ; rm test/symlink.txt && echo success',
